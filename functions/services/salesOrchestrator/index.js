...
export function orchestrateSalesResponse({ message = "", memory = {}, providers = [] } = {}) {
  ...
  let question = discoveryQuestion(p);
  if (question) {
    // A direct recommendation request is a buying signal. When provider and name are
    // already known, collect the monthly bill before continuing broader discovery.
    const isRecommendationRequest = /\b(?:what|which|who)\b.*\b(?:recommend|best|option|provider)\b|\bwhat do you recommend\b/i.test(text);
    if (isRecommendationRequest && p.hasProvider && p.hasName && !p.hasBill) {
      question = {
        action: "ask_monthly_bill",
        message: `Before I narrow the options, about how much are you paying ${p.facts.currentProvider || "your current provider"} each month?`,
        replies: ["$75 a month", "$100 a month", "$125 a month"],
      };
    }

    if (isProviderQuestion(text) && top) {
      // A generic provider "why" interruption continues with issue diagnostics,
      // but the legacy "why did you pick ..." phrasing remains in the normal
      // price-led discovery sequence and collects the monthly bill first.
      const reliabilityLedWhy = /\bwhy\b/i.test(text) && !/\bwhy\s+did\s+you\s+pick\b/i.test(text);
      const needsIssueBeforeBill = question.action === "ask_monthly_bill" && reliabilityLedWhy && !p.facts.issueType && !p.facts.problemDescription;
      const nextAction = needsIssueBeforeBill ? "ask_issue_type" : question.action;
      const nextMessage = needsIssueBeforeBill
        ? "Are the problems affecting the entire connection, or are you mainly experiencing Wi-Fi drops and coverage issues?"
        : question.message;
