...
export async function persistAdvisorLead(payload = {}) {
  ...
  const data = {
    ...
    followUpPermission: typeof facts.followUpPermission === "boolean" ? facts.followUpPermission : (existing.followUpPermission ?? null),
    ...
    salesProfile: {
      currentProvider: facts.currentProvider || existing.currentProvider || "",
      monthlyBill: facts.monthlyBill ?? existing.monthlyBill ?? null,
      reasonForShopping: (memory.painPoints || existing.painPoints || []).join(", "),
      internetUsage: (memory.householdNeeds || existing.usageProfile || []).join(", "),
      decisionPriority: facts.decisionPriority || existing.salesProfile?.decisionPriority || "",
      buyingTimeline: facts.buyingTimeline || facts.switchIntent || existing.salesProfile?.buyingTimeline || "",
      recommendedProvider: quote?.provider || memory.selectedProvider || existing.recommendedProvider || "",
      followUp: facts.followUpPermission === false ? "No phone call" : `Call ${facts.bestContactTime || "as soon as possible"}`,
    },
