...
test("does not recommend the customer's current provider when alternatives exist", () => {
  const response = buildAdvisorResponse({
    routerResult: {
      message: "I have AT&T currently though",
      memory: { facts: { currentProvider: "AT&T", monthlyBill: 115 }, painPoints: ["price"], householdNeeds: ["workFromHome"], preferences: ["price"] },
      orchestration: { nextBestAction: { action: "present_recommendation" } },
    },
    providers: [{ name: "AT&T" }, { name: "Spectrum" }, { name: "Frontier" }],
  });
  assert.match(response.message, /already have AT&T/i);
  assert.match(response.message, /Spectrum/i);
  assert.doesNotMatch(response.message, /AT&T is currently my leading recommendation/i);
});

test("why-best follow-up receives an explanation rather than the identical recommendation", () => {
  const response = buildAdvisorResponse({
    routerResult: {
      message: "Why is it best?",
      memory: { facts: { currentProvider: "AT&T", monthlyBill: 115 }, painPoints: ["price"], householdNeeds: [], preferences: ["price"] },
      orchestration: { nextBestAction: { action: "present_recommendation" } },
    },
    providers: [{ name: "AT&T" }, { name: "Spectrum" }],
  });
  assert.match(response.message, /Spectrum leads because/i);
  assert.match(response.message, /excluding AT&T/i);
});

test("quote builder excludes current provider when an alternative exists", async () => {
  const { buildAdvisorQuote } = await import("../services/aiAdvisor/responseBuilder.js");
  const quote = buildAdvisorQuote({
    routerResult: { memory: { facts: { currentProvider: "AT&T", monthlyBill: 115 } } },
    providers: [{ name: "AT&T" }, { name: "Spectrum", monthlyPrice: 80 }],
  });
  assert.equal(quote.provider, "Spectrum");
  assert.equal(quote.estimatedMonthlySavings, 35);
});
