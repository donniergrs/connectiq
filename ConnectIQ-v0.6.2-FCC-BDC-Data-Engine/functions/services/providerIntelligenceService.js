...
export async function lookupProviderIntelligence(address, options = {}) {
  ...
  let verifiedResult = lookupBdcByAddress(normalizedAddress);
  trace.push({
    source: "fcc-bdc-download",
    status: verifiedResult.status,
    detail: verifiedResult.status === "database_missing"
      ? "FCC BDC database has not been imported"
      : `${verifiedResult.providers.length} verified providers returned`,
  });

  if (verifiedResult.status === "database_missing" && String(process.env.ENABLE_UNDOCUMENTED_FCC_LIVE_FALLBACK || "false").toLowerCase() === "true") {
    try {
      verifiedResult = await lookupLiveFcc(normalizedAddress, trace);
    } catch (error) {
      verifiedResult = { status: error.name === "AbortError" ? "timeout" : "failed", providers: [], error: error.message, attempts: [] };
      trace.push({ source: "fcc-live", status: verifiedResult.status, detail: error.message });
    }
  }

  let aiResult = { status: "not_needed", candidates: [] };
  if (!verifiedResult.providers.length && options.includeAiResearch !== false) {
    try {
  ...
  const source = verifiedResult.providers.length
    ? (verifiedResult.providers[0]?.source || "fcc-bdc-download")
    : "provider-intelligence-waterfall";
  const result = {
    ok: !["failed", "timeout"].includes(verifiedResult.status),
    address: normalizedAddress,
    matchedAddress: verifiedResult.geocode?.matchedAddress || normalizedAddress,
    source,
    providerCount: verifiedResult.providers.length,
    providers: verifiedResult.providers,
    verifiedProviders: verifiedResult.providers,
    aiCandidates: aiResult.candidates,
    candidateCount: aiResult.candidates.length,
    status: verifiedResult.providers.length ? "verified" : verifiedResult.status,
    verificationRequired: !verifiedResult.providers.length,
    recommendationEligible: verifiedResult.providers.length > 0,
    bdc: { dataset: verifiedResult.dataset || getBdcStatus() },
    fcc: verifiedResult.attempts ? { ...verifiedResult, providers: undefined } : undefined,
...
export function providerIntelligenceStatus() {
  return {
    ...
    bdc: getBdcStatus(),
    undocumentedLiveFallbackEnabled: String(process.env.ENABLE_UNDOCUMENTED_FCC_LIVE_FALLBACK || "false").toLowerCase() === "true",
