...
export default function ProviderDiagnostics() {
  ...
  const bdcMissing = false;
  const bdcStatus = null;
  ...
  return (
    <main className="pid-page">
      <header className="pid-header">
        <div>
          <span className="pid-eyebrow">ConnectIQ v0.7.0</span>
          <h1>Provider Intelligence</h1>
          <p>Review DSI-qualified or AI-suggested provider options from the ConnectIQ Intelligence Engine.</p>
        </div>
      ...
      <section className="pid-search-card">
        ...
        <div className={`pid-status-banner ${report ? (report.error ? "danger" : report.audit.hasStaticFallback ? "warning" : verifiedCount ? "success" : "neutral") : "neutral"}`}>
          ...
          <div>
            ...
            <span>{report ? (bdcMissing ? "DSI is not configured, so ConnectIQ used AI research and marked the results for verification." : report.error || report.audit.warnings[0] || `${providerRows.length} provider records returned.`) : "Enter an address to inspect provider availability."}</span>
          </div>
        </div>
      </section>
        ...
        <div className="pid-content-grid">
          <section className="pid-panel pid-provider-panel">
            <div className="pid-panel-heading">
              <div><h2>Provider results <span>({providerRows.length})</span></h2><p>DSI results are verified. AI suggestions remain clearly marked for confirmation before ordering.</p></div>
            ...
            {providerRows.length ? (
              ...
              <div className="pid-empty-state"><RadioTower size={32} /><h3>{bdcMissing ? "Provider verification required" : "No provider options identified"}</h3><p>{bdcMissing ? "DSI and OpenAI did not return provider options. Send the lead to manual verification." : "The result remains empty. ConnectIQ did not insert a default carrier or generate a quote."}</p></div>
            )}
          ...
          <aside className="pid-side-stack">
            ...
            <section className="pid-panel pid-detail-card">
              ...
              <dl className="pid-detail-list">
                ...
                <div><dt>BDC database</dt><dd>{bdcStatus?.ready ? "Ready" : "Not loaded"}</dd></div>
                <div><dt>Dataset version</dt><dd>{bdcStatus?.dataset?.version || "—"}</dd></div>
              </dl>
            </section>
