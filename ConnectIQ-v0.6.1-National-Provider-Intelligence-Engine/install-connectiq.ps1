...
Write-Host "ConnectIQ v0.6.1 installer" -ForegroundColor Cyan
...
& robocopy $target $backup /E /XD node_modules dist .git /XF .env /R:2 /W:1 /NFL /NDL /NJH /NJS
if ($LASTEXITCODE -ge 8) { throw "Backup failed with Robocopy code $LASTEXITCODE" }

Write-Host "[2/6] Installing complete release"
& robocopy $source $target /E /XD node_modules .git release-output /XF .env /R:2 /W:1 /NFL /NDL /NJH /NJS
...
Write-Host "ConnectIQ v0.6.1 installation complete." -ForegroundColor Green
