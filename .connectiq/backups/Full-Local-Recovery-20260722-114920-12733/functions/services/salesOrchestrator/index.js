...
export function orchestrateSalesResponse({ message = "", memory = {}, providers = [], quote = null } = {}) {
  ...
  if (contactPreferenceAnswer) return { stage: "ORDER_READY", nextAction: "create_quote_and_handoff", selectedProvider: top?.provider || null, selectedProviderName: top?.name || null, message: `Perfect. I have your information and your request saved. ConnectIQ will prepare the ${top?.name || "recommended provider"} comparison and follow up using ${p.facts.contactPreference === "any" ? "text, email, or phone" : p.facts.contactPreference}. Is there anything else you would like me to include before I close this conversation?`, suggestedReplies: ["No, that is all", "I have one more question"] };
