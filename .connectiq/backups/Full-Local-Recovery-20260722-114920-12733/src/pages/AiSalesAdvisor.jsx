...
export default function AiSalesAdvisor() {
  ...
  async function lookup(event) {
    ...
    setBusy(true);
    try {
      const activeSessionId = newSessionId();
      setSessionId(activeSessionId);
      localStorage.setItem("connectiqAdvisorSession", activeSessionId);
      setMemory({});
      setQuote(null);
      setConversationStage("DISCOVERY");
      setConversationStatus("active");
      const result = await lookupProviderIntelligence(address.trim());
      const found = Array.isArray(result.providers) ? result.providers : [];
      setProviders(found);
      if (found.length) await ensureAdvisorLead({ sessionId: activeSessionId, address: address.trim(), providers: found });
      setMessages((items) => [...items, { role: "customer", text: address.trim() }, { role: "advisor", text: found.length ? `I found ${found.length} internet provider option${found.length === 1 ? "" : "s"} that may serve your address. Tell me about your current service so I can recommend the best fit. Final serviceability will be confirmed before an order is submitted.` : "I could not identify provider options for that address. Please verify the address and try again." }]);
    } catch (error) {
  ...
  async function submitTurn(text = message) {
    ...
    setBusy(true);
    try {
      const history = [...messages, { role: "customer", text: clean }].slice(-16);
      ...
      const nextAction = result.response?.nextAction || result.orchestration?.nextBestAction?.action || "";
      if (result.advisor?.stage) setConversationStage(result.advisor.stage);
      else if (nextAction === "present_recommendation" || result.quote?.provider) setConversationStage("RECOMMENDATION");
      if (nextAction === "continue_to_order") setConversationStage("ORDER_READY");
      if (nextAction === "complete_conversation" || result.advisor?.stage === "COMPLETED") setConversationStatus("completed");
      ...
      await syncAdvisorLead({
        ...
        advisorMessage,
        stage: result.advisor?.stage || result.stage || conversationStage,
      });
    } catch (error) {
      setMessages((items) => [...items, { role: "advisor", text: error.message }]);
    } finally { setBusy(false); }
  ...
  return <main className="ai004c-page">
    ...
    <section className="ai004c-grid">
      <aside className="ai004c-panel"><h2><Wifi size={19}/> Available providers</h2>
        {!providers.length ? <form onSubmit={lookup} className="ai004c-address"><input value={address} onChange={e=>setAddress(e.target.value)} placeholder="Full service address"/><button disabled={busy}>Check address <ArrowRight size={17}/></button></form> : providers.slice(0,5).map((provider,index)=><article className={`ai004c-provider ${index===0?'is-best':''}`} key={`${providerName(provider)}-${index}`}><div><strong>{providerName(provider)}</strong>{index===0&&<span>Leading option</span>}</div><p>{provider.technology || provider.technologyType || provider.technology_code_type || "Broadband"}</p><small>{provider.evidence || "Potential availability identified by ConnectIQ AI. Final serviceability confirmation is required before ordering."}</small></article>)}
      </aside>
      <section className="ai004c-panel ai004c-chat"><h2><Bot size={19}/> Your advisor</h2><div className="ai004c-messages">{messages.map((item,index)=><div key={index} className={`ai004c-message ${item.role}`}>{item.text}</div>)}{busy&&<div className="ai004c-message advisor"><LoaderCircle className="ai004c-spin" size={17}/> Reviewing your options…</div>}</div>{suggestedReplies.length>0&&<div className="ai004c-suggestions">{suggestedReplies.map(item=><button key={item} onClick={()=>submitTurn(item)}>{item}</button>)}</div>}{conversationStatus === "completed" ? <button className="ai004c-primary" onClick={()=>{ localStorage.removeItem("connectiqAdvisorSession"); window.location.reload(); }}>Start New Conversation</button> : <div className="ai004c-compose"><input value={message} onChange={e=>setMessage(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')submitTurn();}} placeholder="Tell me about your current service…" disabled={busy}/><button onClick={()=>submitTurn()} disabled={busy||!message.trim()}><Send size={18}/></button></div>}</section>
