...
test("accepts any contact method after contact preference question", () => {
  ...
  assert.equal(result.facts.contactPreference, "any");
});

test("moves to order ready immediately after contact preference is captured", () => {
  ...
  assert.equal(decision.stage, "ORDER_READY");
  assert.equal(decision.nextAction, "create_quote_and_handoff");
  assert.match(decision.message, /anything else/i);
});
