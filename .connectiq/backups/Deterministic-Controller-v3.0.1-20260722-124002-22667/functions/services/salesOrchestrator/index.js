...
export function orchestrateSalesResponse({ message = "", memory = {}, providers = [] } = {}) {
  ...
  if (!p.hasPreference) return { stage: "CLOSING", nextAction: "ask_contact_preference", selectedProvider: top?.provider || null, selectedProviderName: top?.name || null, message: "A phone call will be the primary follow-up. May we also contact you by text or email if needed?", suggestedReplies: ["Yes, either is fine", "Text is fine", "Email only"] };
  if (!p.hasBestTime) return { stage: "CLOSING", nextAction: "ask_best_contact_time", selectedProvider: top?.provider || null, selectedProviderName: top?.name || null, message: "Is there a preferred time for the call—morning, afternoon, evening, or simply as soon as possible?", suggestedReplies: ["As soon as possible", "Morning", "Afternoon", "Evening"] };
