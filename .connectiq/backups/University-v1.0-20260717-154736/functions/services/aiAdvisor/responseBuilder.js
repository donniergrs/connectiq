...
function suggestedRepliesForSkill(skill = "") {
  const map = {
    PricingSkill: ["Compare with my current bill", "Show alternatives", "Build my quote"],
    MobileSkill: ["Include mobile bundles", "Continue internet comparison", "Show alternatives"],
    InstallationSkill: ["Prepare installation details", "Build my quote", "Show alternatives"],
    ComparisonSkill: ["Compare monthly cost", "Compare reliability", "Why is the first one best?"],
    ExplanationSkill: ["Show alternatives", "Compare monthly cost", "Build my quote"],
    QuoteSkill: ["Continue to order details", "Show alternatives", "Talk to an advisor"],
    CorrectionSkill: ["Compare price", "Compare speed", "Compare reliability"],
    RecommendationSkill: ["Why is it best?", "Show alternatives", "Build my quote"],
    DiscoverySkill: ["How much is it?", "Why is it best?", "Show alternatives"],
    HumanHandoffSkill: ["Call me", "Text me", "Email me"],
  };
  return map[skill] || ["Compare providers", "Ask about pricing", "Talk to an advisor"];
}
...
export function buildAdvisorResponse({ routerResult = {}, providers = [], quote = null } = {}) {
  // Brain v2 responses take precedence over the legacy recommendation fallback.
  if (routerResult.brainV2?.response) {
    const result = routerResult.brainV2.response;
    return {
      message: [result.answer, result.followUp].filter(Boolean).join(" "),
      suggestedReplies: suggestedRepliesForSkill(result.skill),
      skill: result.skill,
      intent: routerResult.brainV2.intent?.primary || null,
      brainVersion: routerResult.brainV2.version,
    };
  }
