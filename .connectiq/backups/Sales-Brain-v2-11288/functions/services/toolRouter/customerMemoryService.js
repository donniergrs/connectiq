...
export function hydrateCustomerMemory(sessionId, snapshot = {}) {
  if (!sessionId || !snapshot || typeof snapshot !== "object") return getCustomerMemory(sessionId);
  const current = getCustomerMemory(sessionId);
  const hydrated = {
    ...current,
    ...snapshot,
    sessionId,
    facts: { ...current.facts, ...(snapshot.facts || {}) },
    confidence: { ...current.confidence, ...(snapshot.confidence || {}) },
    painPoints: [...new Set([...(current.painPoints || []), ...(snapshot.painPoints || [])])],
    householdNeeds: [...new Set([...(current.householdNeeds || []), ...(snapshot.householdNeeds || [])])],
    preferences: [...new Set([...(current.preferences || []), ...(snapshot.preferences || [])])],
    rejectedProviders: [...new Set([...(current.rejectedProviders || []), ...(snapshot.rejectedProviders || [])])],
    recentTurns: Array.isArray(snapshot.recentTurns) ? snapshot.recentTurns.slice(-16) : (current.recentTurns || []),
    updatedAt: new Date().toISOString(),
  };
...
export function extractFactsFromMessage(message = "", currentMemory = {}) {
  ...
  const nameMatch = text.match(/^\s*(?:my name is|this is)\s+([a-z][a-z' -]{1,40})[.!?]*\s*$/i);
  if (nameMatch) {
    facts.customerName = nameMatch[1].trim().replace(/\b\w/g, (c) => c.toUpperCase());
    facts.preferredName = facts.customerName.split(/\s+/)[0];
    confidence.customerName = 0.94;
  }
