...
app.get("/api/conversations/advisor/health", (req, res) => {
  res.json({ ok: true, service: "connectiq-sales-closer-conversation-engine", version: "RC2-RC3-RC4-v1.0.20", mode: "unified_sales_orchestrator" });
});
...
app.post("/api/conversations/advisor/turn", async (req, res) => {
  try {
    ...
    const advisor = {
      ...
      conversationMode: "unified_sales_orchestrator",
    };
    res.json({ ...routerResult, salesIntelligence, advisor, quote, enterprise, salesDecision, conversationMode: "unified_sales_orchestrator" });
  } catch (error) {
    res.status(400).json({ ok: false, error: error.message });
  }
});
