...
app.post("/api/conversations/advisor/turn", async (req, res) => {
  try {
    ...
    const routerResult = await routeConversationTurn({ ...(req.body || {}), context: { ...(req.body?.context || {}), providers, address: req.body?.address || "" } });
...
registerCustomerTwinRoutes(app, typeof db !== "undefined" ? db : null);



registerSemanticUnderstandingRoutes(app, typeof db !== "undefined" ? db : null);
