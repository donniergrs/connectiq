...
export function updateCustomerMemory(sessionId, patch = {}) {
  ...
  const next = {
    ...
    journal: patch.journal || current.journal || [],
    conversationState: { ...(current.conversationState || {}), ...(patch.conversationState || {}) },
...
export function extractFactsFromMessage(message = "") {
  ...
  const preferences = [];
  if (/^\s*(?:lowest\s+)?price\s*[.!?]*$/i.test(text)) preferences.push("price");
  if (/^\s*(?:best\s+)?reliability\s*[.!?]*$/i.test(text)) preferences.push("reliability");
  if (/^\s*(?:fastest\s+)?speed\s*[.!?]*$/i.test(text)) preferences.push("speed");
  if (/^\s*wi-?fi(?: coverage)?\s*[.!?]*$/i.test(text)) preferences.push("wifiCoverage");
  return { facts, confidence, painPoints, householdNeeds, preferences };
}
