...
import { learnFromMessage, getCustomerMemory, updateCustomerMemory } from "./customerMemoryService.js";
...
import { orchestrateTurn } from "../conversationOrchestrator/index.js";
...
export async function routeConversationTurn({ sessionId, message, stage = "DISCOVERY", context = {} }) {
  ...
  const orchestration = orchestrateTurn({ message, memory, providers, stage, semantic: intent });
  const enrichedMemory = updateCustomerMemory(sessionId, {
    ...
    decisions: [...(memory.decisions || []), { at: new Date().toISOString(), ...orchestration.decision }].slice(-100),
  });
  ...
  const turn = {
    ...
    orchestration,
    response: { ...response, nextAction: orchestration.decision.action, decisionReason: orchestration.decision.reason },
