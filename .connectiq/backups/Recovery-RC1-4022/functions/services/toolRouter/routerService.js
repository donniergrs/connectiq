...
import { inferStage, isCompletionIntent } from "../salesCloser/conversationGuard.js";
...
export async function routeConversationTurn({ sessionId, message, stage = "DISCOVERY", context = {}, address = "" }) {
  ...
  const effectiveStage = inferStage(memory, context.quote || null, stage, message);
  const completion = effectiveStage === "COMPLETED" || isCompletionIntent(message) && memory.status === "COMPLETED";
  const stagedMemory = updateCustomerMemory(sessionId, {
    stage: effectiveStage,
    status: completion ? "COMPLETED" : (memory.status || "ACTIVE"),
    completedAt: completion ? (memory.completedAt || new Date().toISOString()) : memory.completedAt,
  });
  const intent = analyzeIntent(message, { ...context, stage: effectiveStage, memory: stagedMemory });
  ...
  const orchestration = orchestrateTurn({ sessionId, message, intent, learned, memory: stagedMemory, providers });
  ...
  const pipeline = await executeTools(toolNames, { sessionId, message, stage: effectiveStage, context, intent, learned, memory: enrichedMemory, orchestration });
