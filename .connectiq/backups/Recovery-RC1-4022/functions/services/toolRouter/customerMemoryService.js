...
export function updateCustomerMemory(sessionId, patch = {}) {
  ...
  const next = {
    ...
    recentTurns: [...(current.recentTurns || []), ...(patch.recentTurns || [])].slice(-24),
    stage: patch.stage || current.stage || "INTRODUCTION",
    status: patch.status || current.status || "ACTIVE",
    askedFields: [...new Set([...(current.askedFields || []), ...(patch.askedFields || [])])],
