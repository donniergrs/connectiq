...
export default function AiSalesAdvisor() {
  ...
  async function submitTurn(text = message) {
    ...
    setBusy(true);
    try {
      const result = await sendAdvisorTurn({ sessionId, stage, message: clean, providers, address, quote });
      const advisorMessage = result.advisor?.message || result.response?.message || "Let’s continue.";
      setMemory(result.memory || {});
      setQuote(result.quote || null);
      setStage(result.stage || result.memory?.stage || stage);
      setConversationStatus(result.status || result.memory?.status || "ACTIVE");
