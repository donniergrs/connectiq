...
import { persistAdvisorLead } from "./services/advisorLeadPersistence.js";
...
app.post("/api/leads/advisor/sync", async (req, res) => {
  try {
    const result = await persistAdvisorLead(req.body || {});
    return res.status(result.created ? 201 : 200).json({ ok: true, ...result });
  } catch (error) {
    console.error("Advisor lead persistence failed", error);
    return res.status(500).json({ ok: false, error: error.message || "Unable to save advisor lead." });
  }
});

// CONNECTIQ-AI-004C-ROUTES
app.get("/api/conversations/advisor/health", (req, res) => {
  res.json({ ok: true, service: "connectiq-sales-closer-conversation-engine", version: "RC3.7-discovery-recovery", mode: "adaptive_sales_closer" });
});
...
app.post("/api/conversations/advisor/turn", async (req, res) => {
  try {
    ...
    const advisor = {
      ...
      conversationMode: "adaptive_sales_closer",
    };
    res.json({ ...routerResult, salesIntelligence, advisor, quote, enterprise, conversationMode: "adaptive_sales_closer" });
  } catch (error) {
    res.status(400).json({ ok: false, error: error.message });
  }
});
