...
export function orchestrateSalesResponse({ message = "", memory = {}, providers = [], quote = null } = {}) {
  ...
  if (contactPreferenceAnswer) return { stage: "ORDER_READY", nextAction: "create_quote_and_handoff", selectedProvider: top?.provider || null, selectedProviderName: top?.name || null, message: `Perfect. I have your information and requirements saved. A ConnectIQ advisor can now verify the exact plans and prices for ${top?.name || "the recommended providers"} and follow up by ${p.facts.contactPreference === "any" ? "text, email, or phone" : p.facts.contactPreference}. Is there anything else you want highlighted before I close the chat?`, suggestedReplies: ["No, that is all", "I have one more question"] };
  ...
  const explicitlyRequestsRecommendation = is(text, /what do you suggest|what.*recommend|best option|which.*choose|what should i do|compare the first two|which is most reliable|why did you pick|why.*recommend/);
  if (question) {
    const prefix = explicitlyRequestsRecommendation ? "I can explain the recommendation, but I need one more detail so I do not steer you to the wrong provider. " : "";
    return { stage: "DISCOVERY", nextAction: question.action, selectedProvider: null, message: `${prefix}${question.message}`, suggestedReplies: question.replies };
  }
