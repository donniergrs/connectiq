...
export default function ProviderDiagnostics() {
  ...
  const bdcMissing = report?.raw?.status === "database_missing";
  const bdcStatus = report?.raw?.bdc?.dataset || null;
  const lookupStatus = !report
    ? "Not run"
    : bdcMissing
      ? "FCC BDC database not loaded"
      : report.error
      ? "Lookup failed"
  ...
  return (
    <main className="pid-page">
      <header className="pid-header">
        <div>
          <span className="pid-eyebrow">ConnectIQ v0.6.2</span>
      ...
      <section className="pid-search-card">
        ...
        <div className={`pid-status-banner ${report ? (report.error ? "danger" : report.audit.hasStaticFallback ? "warning" : verifiedCount ? "success" : "neutral") : "neutral"}`}>
          ...
          <div>
            ...
            <span>{report ? (bdcMissing ? "Import the FCC BDC location and fixed-availability files before running verified lookups." : report.error || report.audit.warnings[0] || `${providerRows.length} provider records returned.`) : "Enter an address to inspect provider availability."}</span>
          </div>
        </div>
      </section>
        ...
        <div className="pid-content-grid">
          <section className="pid-panel pid-provider-panel">
            ...
            {providerRows.length ? (
              ...
              <div className="pid-empty-state"><RadioTower size={32} /><h3>{bdcMissing ? "FCC BDC database not loaded" : "No providers returned"}</h3><p>{bdcMissing ? "Run the FCC BDC import described in docs/FCC_BDC_SETUP.md. Lead cards can still be created while provider enrichment remains queued." : "The result remains empty. ConnectIQ did not insert a default carrier or generate a quote."}</p></div>
            )}
