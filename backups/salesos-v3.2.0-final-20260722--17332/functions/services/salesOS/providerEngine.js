...
export function rankProviders(providers = [], memory = {}) {
  const currentProvider = memory.facts?.currentProvider || memory.currentProvider || memory.customerProfile?.currentProvider || "";
  const currentFamily = providerFamily(currentProvider);
  const rejected = Array.isArray(memory.rejectedProviders) ? memory.rejectedProviders : [];
  const rejectedFamilies = new Set(rejected.map(providerFamily));
  return providers
    .filter((provider) => {
