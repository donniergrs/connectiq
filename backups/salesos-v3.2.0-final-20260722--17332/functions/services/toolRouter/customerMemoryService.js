...
const FACT_PATTERNS = [
  ...
  { key: "monthlyBill", regex: /(?:\$\s*)?(\d{2,4}(?:\.\d{1,2})?)\s*(?:dollars?\s*)?(?:(?:a|per)\s*)?(?:month|monthly|\/mo)\b/i, cast: Number },
...
export function extractFactsFromMessage(message = "", currentMemory = {}) {
  ...
  const lastAction = currentMemory.lastNextAction || "";
  if (lastAction === "ask_issue_type" && text) {
    facts.issueType = /whole|entire|goes out|outage|disconnect/i.test(text) ? "connection_outage" : /wi-?fi|coverage|signal|dead zone/i.test(text) ? "wifi_only" : /buffer|slow|lag/i.test(text) ? "performance" : "other";
    facts.problemDescription = text;
    confidence.issueType = 0.88;
  }
  if (lastAction === "ask_issue_frequency" && text) {
    facts.issueFrequency = text;
    confidence.issueFrequency = 0.86;
  }
  if (lastAction === "ask_customer_impact" && text) {
    facts.usageImpact = text;
    if (/work|meeting|zoom|teams|vpn|business/i.test(text)) facts.businessImpact = text;
    confidence.usageImpact = 0.86;
  }
  if (lastAction === "ask_decision_priority" && text) {
    if (/all three|price.*reliab.*speed|reliab.*speed.*price|balance of all/i.test(text)) {
      facts.decisionPriority = "balance";
      facts.decisionPriorities = ["price", "reliability", "speed"];
      confidence.decisionPriorities = 0.98;
    } else if (/cheap|price|bill|save|lower/i.test(text)) {
      facts.decisionPriority = "price";
      facts.decisionPriorities = ["price"];
    } else if (/reliab|stable|uptime/i.test(text)) {
      facts.decisionPriority = "reliability";
      facts.decisionPriorities = ["reliability"];
    } else if (/fast|speed|bandwidth|download|upload|latency|ping/i.test(text)) {
      facts.decisionPriority = "speed";
      facts.decisionPriorities = ["speed"];
    } else {
      facts.decisionPriority = "balance";
      facts.decisionPriorities = ["price", "reliability", "speed"];
    }
    confidence.decisionPriority = 0.9;
  }
  if (lastAction === "ask_troubleshooting" && text) {
    facts.contactedProvider = /yes|already|called|contacted|they/i.test(text) && !/^no\b/i.test(text);
    facts.troubleshootingStatus = text;
    confidence.troubleshootingStatus = 0.84;
  }
  if (lastAction === "ask_switch_intent" && text) {
    facts.switchIntent = text;
    facts.buyingTimeline = text;
    confidence.switchIntent = 0.86;
  }
  // SalesOS v3.1.0 comparison intent memory. This preserves a request such as
  // "pricing" as a comparison goal instead of incorrectly selecting provider #1.
  if (["present_options", "ask_option_interest", "ask_comparison_interest"].includes(lastAction) && text) {
    const lower = text.toLowerCase();
    const comparisonIntent = /cheapest|lowest price|least expensive|save the most/.test(lower) ? "cheapest"
      : /pricing|prices?|cost|monthly|fees?|how much/.test(lower) ? "price"
      : /most reliable|reliability|reliable|uptime|outage/.test(lower) ? "reliability"
      : /fastest|speed|download|upload/.test(lower) ? "speed"
      : /best value|value|balance/.test(lower) ? "value"
      : /fiber only|only fiber|want fiber/.test(lower) ? "fiber"
      : /cable only|only cable/.test(lower) ? "cable"
      : null;
    if (comparisonIntent) {
      facts.comparisonIntent = comparisonIntent;
      facts.comparisonRequested = true;
      confidence.comparisonIntent = 0.97;
    }
  }
  if (lastAction === "ask_best_contact_time" && text) {
    facts.bestContactTime = /as soon|asap|any time|anytime/i.test(text) ? "as soon as possible" : /morning/i.test(text) ? "morning" : /afternoon/i.test(text) ? "afternoon" : /evening|night/i.test(text) ? "evening" : text;
    confidence.bestContactTime = 0.94;
  }
  if (lastAction === "ask_followup_permission" && text) {
    facts.followUpPermission = !/^(?:no|nope|do not|don'?t|not by phone)/i.test(text);
    confidence.followUpPermission = 0.97;
  }

  // Deterministic contact-state capture. The application, not the model,
  // owns completion of this step so simple answers cannot cause a loop.
  if (lastAction === "ask_contact_preference" && text) {
    const declinedExtraChannels = /^(?:no|nope|phone only|call only|just call|do not text|don'?t text|do not email|don'?t email)[.! ]*$/i.test(text);
    const allowsBoth = /^(?:yes|yeah|yep|sure|ok|okay|that works|both(?: work)?|either(?: one)?(?: works)?|any(?: one| of those)?|all(?: of them| of those)?|text and email|email and text)[.! ]*$/i.test(text);
    const textOnly = /^(?:text|text only|sms|sms only)[.! ]*$/i.test(text);
    const emailOnly = /^(?:email|email only)[.! ]*$/i.test(text);

    if (declinedExtraChannels) {
      facts.contactPreference = "phone";
      facts.followUpPermission = true;
    } else if (allowsBoth) {
      facts.contactPreference = "text_email";
      facts.followUpPermission = true;
    } else if (textOnly) {
      facts.contactPreference = "text";
      facts.followUpPermission = true;
    } else if (emailOnly) {
      facts.contactPreference = "email";
      facts.followUpPermission = true;
    }

    if (facts.contactPreference) {
      confidence.contactPreference = 0.99;
      confidence.followUpPermission = 0.99;
    }
  }

  const advisorAskedContactPreference = /\b(?:prefer|text, email|email, or|phone call|how.*contact|also contact.*text|also contact.*email)\b/i.test(currentMemory.recentTurns?.filter((turn) => turn.role === "advisor").slice(-1)[0]?.message || "") || currentMemory.lastNextAction === "ask_contact_preference";
  if (!facts.contactPreference && advisorAskedContactPreference && /any(?:one| of those)?|either|all (?:of )?(?:them|those)|doesn'?t matter|no preference|both work/i.test(text)) {
    facts.contactPreference = "text_email";
    facts.followUpPermission = true;
    confidence.contactPreference = 0.96;
    confidence.followUpPermission = 0.96;
  }
  ...
  // Contextual bill answer after the advisor explicitly asked for monthly cost.
  if (!facts.monthlyBill && currentMemory.lastNextAction === "ask_monthly_bill" && !currentMemory.facts?.monthlyBill) {
    const billAnswer = text.match(/\$?\s*(\d{2,4}(?:\.\d{1,2})?)(?:\s*dollars?)?(?:\s*(?:a|per)\s*month)?/i);
    if (billAnswer?.[1]) {
      facts.monthlyBill = Number(billAnswer[1]);
      confidence.monthlyBill = 0.97;
    }
  }
