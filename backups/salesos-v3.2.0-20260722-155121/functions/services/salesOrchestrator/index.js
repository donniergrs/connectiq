...
function closeMessage(p, selected, ranked, comparisonIntent) {
  ...
  const usage = [...new Set([...(p.needs || []), f.usageImpact].filter(Boolean))];
  ...
  return `Thanks, ${name}. You're currently with ${f.currentProvider || "your current provider"}${f.monthlyBill ? ` and paying about $${f.monthlyBill} per month` : ""}. You want ${priorities.length ? priorities.join(", ") : "better overall value"} for ${usage.length ? usage.join(", ") : "your household's internet use"}. The advisor will focus on ${focus || "the best eligible alternatives"} and call you ${timing} to verify exact serviceability, plans, pricing, fees, and next steps. Nothing will be submitted without your approval.`;
}
