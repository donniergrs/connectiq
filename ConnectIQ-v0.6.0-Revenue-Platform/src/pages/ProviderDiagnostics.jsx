    ...
    <main className="pid-page">
      ...
      <section className="pid-kpi-grid" aria-label="Diagnostic summary">
        ...
        <article className="pid-kpi-card">
          ...
          <div><span>Best verified option</span><strong className="pid-kpi-provider">{bestProvider?.displayName || "None"}</strong><p>{bestProvider ? formatSpeed(bestProvider.download, bestProvider.upload) : "No fallback used"}</p></div>
        </article>
        <article className="pid-kpi-card">
      ...
      {report && (
        <div className="pid-content-grid">
          <section className="pid-panel pid-provider-panel">
            ...
            <div className="pid-data-note"><CircleAlert size={17} /><p><strong>Data-source rule:</strong> static carrier profiles may provide sales metadata, but they never establish address availability.</p></div>
          </section>
